...
public class LogViewTabComponent extends CustomComponent {
	...
	public LogViewTabComponent(LogViewComponent logView, int id, String name) {
		...
		filterPrefs = sharedPrefs.node("settings." + id);

		container = new FilterSettingContainer(sharedPrefs, filterPrefs);
		...
		mainLayout.setSplitPosition(filterPrefs.getInt("splitter.filter", sharedPrefs.getInt("splitter.filter", 250)),
			Unit.PIXELS);
